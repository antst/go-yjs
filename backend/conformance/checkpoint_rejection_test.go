...
func TestCheckpointSuiteRejectsViolations(t *testing.T) {
	...
	if len(backendtest.AllCheckpointViolations) == 0 || len(backendtest.AllDeletionViolations) == 0 ||
		len(backendtest.AllFencedDeletionViolations) == 0 {
	...
	cases = append(cases, backendtest.AllFencedDeletionViolations...)
...
func runViolationFixture(t *testing.T) {
	...
	for _, fenced := range backendtest.AllFencedDeletionViolations {
		if violation == fenced {
