...
// Invalidate must wait for an abandoned initialization of the same document.
// Detaching such a generation is what stops a later caller joining it, but it
// also takes it out of the map Invalidate looks in — so without explicit
// tracking Invalidate would report success while a load of the document it is
// about to have deleted is still running.
func TestInvalidateWaitsForAnAbandonedOpenOfTheSameDocument(t *testing.T) {
	registry := NewRegistry()
	defer closeRegistry(t, registry)

	entered := make(chan struct{})
	release := make(chan struct{})
	// Deliberately ignores cancellation: the point is that Invalidate waits for
	// the open to actually finish, not merely that it asked it to stop.
	open := func(context.Context) (*crdt.Doc, error) {
		close(entered)
		<-release
		return crdt.NewDoc("doc"), nil
	...
	registry.mu.Lock()
	detached := registry.entries["doc"] == nil
	draining := len(registry.draining)
	registry.mu.Unlock()
	if !detached || draining != 1 {
		t.Fatalf("setup: detached=%v draining=%d, want true/1", detached, draining)
	}
	...
	select {
	case err := <-invalidated:
		t.Fatalf("Invalidate returned (%v) while an abandoned open for the same document was still running", err)
	...
	case <-time.After(5 * time.Second):
		t.Fatal("Invalidate never returned after the abandoned open finished")
	}
}

// The same wait when a LIVE generation also exists. An abandoned open and a
// fresh acquisition of the same document coexist by design — detaching the
// abandoned one is precisely what lets the next caller open a new one — so
// Invalidate has to drain both, not just the one it can find in the map.
func TestInvalidateWaitsForBothTheLiveAndTheAbandonedGeneration(t *testing.T) {
	registry := NewRegistry()
	defer closeRegistry(t, registry)
	...
	go func() { _, err := registry.Acquire(ctx, "doc", open); abandoned <- err }()
	<-firstEntered
	waitForWaiters(t, registry, "doc", 1)
	cancel()
	...
	// A live generation for the same document, alongside the abandoned one.
	live, err := registry.Acquire(context.Background(), "doc", open)
	if err != nil {
		t.Fatal(err)
	}
	registry.mu.Lock()
	haveLive := registry.entries["doc"] != nil
	orphans := len(registry.draining)
	registry.mu.Unlock()
	if !haveLive || orphans != 1 {
		t.Fatalf("setup: live=%v orphans=%d, want true/1", haveLive, orphans)
	}

	invalidated := make(chan error, 1)
	go func() { invalidated <- registry.Invalidate(context.Background(), "doc") }()

	// Releasing the live handle satisfies only half of what Invalidate waits on.
	<-live.Done()
	live.Release()
	select {
	case err := <-invalidated:
		t.Fatalf("Invalidate returned (%v) with the abandoned open still running", err)
	case <-time.After(100 * time.Millisecond):
	}
	...
	case <-time.After(5 * time.Second):
		t.Fatal("Invalidate never returned after both generations finished")
	}
}
