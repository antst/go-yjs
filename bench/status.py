...
# GitHub and Forgejo both render Markdown and neither renders a tracked .html file --
# the repo browser shows source and the raw endpoint serves text/plain, on purpose. So the
# page that ships in docs/ is Markdown; the HTML renderer stays for local viewing. Both are
# generated from the same rows, so they cannot disagree.
MD_MARK = {"win": "\N{LARGE GREEN CIRCLE}", "parity": "\N{LARGE YELLOW CIRCLE}",
           "loss": "\N{LARGE RED CIRCLE}"}


def markdown(rows, s, meta):
    def cell(r, label):
        ratio, verdict = r["cells"][label]
        ...
        if verdict == "na":
            ...
            return f"n/a{'' if not why else ' *'}"
        if verdict == "noref":
            return "pending"
        return f"{MD_MARK[verdict]} {ratio:.2f}×"

    out = ["# Performance Status", "",
           "*Generated by `bench/status.py` — do not hand-edit.*", "",
           "Every operation the differential oracle tracks, measured against the yjs reference, "
           "yrs and reearth/ygo **on one machine in one session**. Ratios are reference ÷ ours, "
           "so above 1.00× means this library is faster.", "",
           f"{MD_MARK['win']} clear win (above {PARITY_HIGH:.2f}×) &nbsp;·&nbsp; "
           f"{MD_MARK['parity']} parity ({PARITY_LOW:.2f}–{PARITY_HIGH:.2f}×, inside run-to-run spread) "
           f"&nbsp;·&nbsp; {MD_MARK['loss']} clear loss (below {PARITY_LOW:.2f}×)", "",
           f"`{s['measured']}/{s['total']}` operations measured &nbsp;·&nbsp; "
           f"`{s['fastest']}/{s['ranked']}` faster than every competitor &nbsp;·&nbsp; "
           f"`{len(s['losses'])}` losses outstanding", ""]
    for line in meta:
        out.append(f"> {line}")
    out.append("")
    ...
    for r in rows:
        if r["cat"] != cat:
            ...
            out += ["", f"## {cat}", "",
                    ...
                    "|---|---:|---:|---:|---:|---:|"]
        ours = f'{r["ours"]/1e6:.4f}' if r["ours"] else "unmeasured"
        if r["best_ratio"] is None:
            best = "—"
        else:
            bv = ("loss" if r["best_ratio"] < PARITY_LOW
                  else "parity" if r["best_ratio"] <= PARITY_HIGH else "win")
            best = f"{MD_MARK[bv]} {r['best_ratio']:.2f}×"
        nm = f"`{r['name']}`"
        if r["name"] in UNIT_NOTES:
            nm += " ᵘ"
        if r["name"] in STALE_ROWS:
            nm += " ⚠"
        out.append(f"| {nm} | {ours} | {best} | {cell(r,'yjs')} | {cell(r,'yrs')} | {cell(r,'ygo')} |")

    out += ["", "## Losses", ""]
    if s["losses"]:
        for n, l, rt in s["losses"]:
            out.append(f"- `{n}` vs {l} — **{1/rt:.1f}× slower**")
    else:
        out.append("- none")
    if any(r["name"] in UNIT_NOTES for r in rows):
        out += ["", "ᵘ the two sides do not use the same unit of work for this row; "
                "see `UNIT_NOTES` in `bench/status.py`."]
    if any(r["name"] in STALE_ROWS for r in rows):
        out += ["", "⚠ row measured against a superseded fixture; "
                "see `STALE_ROWS` in `bench/status.py`."]
    out.append("")
    return "\n".join(out)
...
def main():
    ...
    ap.add_argument("-o", "--output", default=None,
                    help="write the page here; .md renders Markdown, anything else HTML")
    ...
    if args.output:
        ...
        as_md = args.output.lower().endswith(".md")
        page = (markdown if as_md else html)(rows, s, meta or [args.outdir])
        # --allow-stale suppresses the embedded banner too, not just the exit code.
        # The tracked page in docs/ is generated at commit X and then committed, which
        # produces commit Y -- so it is one commit "stale" by construction, permanently.
        # The terminal WARNING below is still printed unconditionally, so an interactive
        # run always says so even when the page does not carry it.
        show_stale = stale and not args.allow_stale
        if show_stale and as_md:
            page = ("> **STALE — DO NOT QUOTE THESE NUMBERS:** " + stale
                    + ". Regenerate from a run at the current commit.\n\n" + page)
        elif show_stale:
            page = page.replace(
                '<div class="wrap">',
