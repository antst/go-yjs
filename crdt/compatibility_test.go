...
func TestMapSet(t *testing.T) {
	...
	doc := newDoc("guid", false, nil, nil, false, WithClientID(440166001))
	x := doc.GetMap("test")
	doc.Transact(func(trans *Transaction) {
		x.Set("k1", "v1")
		x.Set("k2", "v2")
...
func TestArrayInsert(t *testing.T) {
	...
	// construct doc by golang and check to see if the result is the same as the expected.
	// Pin the clientID to the JS fixture's value deterministically (no mockey).
	doc := newDoc("guid", false, nil, nil, false, WithClientID(2525665872))
	x := doc.GetArray("test")
	x.Push([]any{"a"})
	x.Push([]any{"b"})
...
func TestStateVector(t *testing.T) {
	...
	sv := mustSV(decodeStateVector(payload))
	...
	serialized := encodeStateVectorWith(nil, sv, newUpdateEncoderV1())
